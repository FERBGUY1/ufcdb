...
import { getPrediction, search, fmtRecord, fmtOdds } from '../lib/api';
...
export default function PredictPage() {
  ...
  const predict = async () => {
    ...
    try {
      const data = await getPrediction(f1.slug, f2.slug);
      setPrediction(data);
    } catch (e) {
    ...
    <main className="max-w-5xl mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="page-sub">
          Select any two fighters from the database. Our engine analyzes styles, stats, cardio,
          opponent quality, and historical matchup data to generate a data-driven prediction.
        </p>
        <div className="text-xs text-white/20 bg-dark-3 border border-white/[0.06] rounded-lg px-4 py-3 mt-3">
          ⚠️ Predictions are generated by comparing historical fight data and statistics.
          They are not a guarantee of outcome and should not be used for betting purposes.
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-4 items-start mb-6">
        <FighterSelector
          ...
          selected={f1}
          onSelect={setF1}
          onClear={() => { setF1(null); setPrediction(null); }}
        />
        ...
        <FighterSelector
          ...
          selected={f2}
          onSelect={setF2}
          onClear={() => { setF2(null); setPrediction(null); }}
        />
      </div>
        ...
        <button
          ...
          className={`font-display tracking-widest text-base px-10 py-3 rounded-lg transition-all duration-200 ${
            canPredict && !loading
              ...
              : 'bg-dark-4 text-white/20 cursor-not-allowed'
          }`}
        >
      ...
      {prediction && !loading && (
        <PredictionResult prediction={prediction} f1Name={f1} f2Name={f2} />
      )}
    </main>
  );
}
  ...
  if (selected) {
    return (
      <div className="card p-5 text-center">
        ...
        <button onClick={onClear} className="mt-3 text-xs text-white/30 hover:text-white/60 transition-colors">
          Change fighter
        </button>
      </div>
    );
              ...
              <button key={f.id} onClick={() => pick(f)}
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-white/40">
                    {fmtRecord(f.wins,f.losses,f.draws)} · {f.primary_style || 'Unknown style'}
                  </div>
                </div>
...
function PredictionResult({ prediction: p, f1Name, f2Name }) {
  ...
  const favorite = f1WinPct > f2WinPct ? f1 : f2;
  const favPct   = Math.max(f1WinPct, f2WinPct);

  const confidenceColor = {
    high: 'text-win', medium: 'text-gold', low: 'text-white/40'
  }[p.confidence] || 'text-white/40';
    ...
    <div className="space-y-5">
      ...
      <div className="card p-6">
        <div className="text-[10px] tracking-[0.3em] text-gold uppercase text-center mb-4">
          Win Probability
          <span className={`ml-3 ${confidenceColor}`}>· {p.confidence?.toUpperCase()} CONFIDENCE</span>
        </div>

        <div className="grid grid-cols-[1fr_auto_1fr] gap-4 items-center mb-5">
          <div className="text-center">
            <div className="font-display text-4xl tracking-wider text-white">{f1WinPct}%</div>
            <Link to={`/fighters/${f1?.slug}`} className="text-sm font-medium hover:text-gold transition-colors">
          ...
          <div className="text-center">
            <div className="font-display text-4xl tracking-wider text-white">{f2WinPct}%</div>
            <Link to={`/fighters/${f2?.slug}`} className="text-sm font-medium hover:text-gold transition-colors">
        ...
        <div className="relative h-3 bg-dark-5 rounded-full overflow-hidden">
          <div className="absolute left-0 top-0 h-full bg-gold rounded-full transition-all duration-1000"
            style={{ width: `${f1WinPct}%` }} />
          <div className="absolute right-0 top-0 h-full bg-red-700/70 rounded-full transition-all duration-1000"
            style={{ width: `${f2WinPct}%` }} />
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="card p-5">
          <div className="text-[10px] tracking-[0.2em] text-white/30 uppercase mb-4">
            {f1?.first_name} {f1?.last_name} wins by...
          </div>
        ...
        <div className="card p-5">
          <div className="text-[10px] tracking-[0.2em] text-white/30 uppercase mb-4">
            {f2?.first_name} {f2?.last_name} wins by...
          </div>
          <div className="space-y-3">
      ...
      {p.round_projections?.length > 0 && (
        <div className="card p-5">
            ...
            {p.round_projections.map(r => (
              <div key={r.round} className="grid grid-cols-[auto_1fr_auto] gap-3 items-center">
                ...
                <div className="relative h-2 bg-dark-5 rounded-full overflow-hidden">
                  <div className="absolute left-0 top-0 h-full bg-gold/60 rounded-full"
                    style={{ width: `${r.f1_control_pct}%` }} />
                </div>
                <span className="text-xs text-white/40 text-right w-24 truncate">
                  {r.projected_control}
                </span>
              </div>
            ))}
          ...
          <div className="flex justify-between mt-3 text-[10px] text-white/20">
            <span>{f1?.last_name} ←</span>
            <span>→ {f2?.last_name}</span>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-3 justify-center pt-2">
        <Link to={`/fighters/${f1?.slug}`} className="btn-outline text-xs">
          {f1?.last_name} Profile
        </Link>
        <Link to={`/fighters/${f2?.slug}`} className="btn-outline text-xs">
          {f2?.last_name} Profile
        </Link>
        <Link to={`/compare?f1=${f1?.slug}&f2=${f2?.slug}`} className="btn-outline text-xs">
          Full Comparison
        </Link>
      </div>
    </div>
