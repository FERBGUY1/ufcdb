...
// Key factors arrive from the engine already phrased from the advantaged
// fighter's side, shaped "{sentence} · {N} pts". Show the sentence as the row
// text and the point contribution as a gold chip on the right.
function KeyFactorRow({ rank, factor }) {
  const m = factor.match(/^(.*?)\s*·\s*(\d+)\s*pts\s*$/i);
  const detail = m ? m[1].trim() : factor;
  const pts = m ? m[2] : null;
    ...
    <div className="space-y-5">
      ...
      <div className="card p-6">
        ...
        <p className="text-[10px] text-white/25 leading-relaxed mt-4 pt-3 border-t border-white/[0.04] text-center">
          Win probability from the UFCDB logistic model ({p.model_version}), trained on per-fight UFC statistics —
      ...
      {p.ai_breakdown && (
        <div className="card p-6">
          ...
          <div className="mt-4 pt-4 border-t border-white/[0.04] text-[10px] text-white/20">
            Generated by UFCDB prediction model {p.model_version} · {new Date(p.generated_at).toLocaleDateString()}
          </div>
        </div>
      )}
